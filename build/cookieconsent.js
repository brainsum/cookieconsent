  ...
  static lightenDarkenColor(col, amt) {
    ...
    value: function lightenDarkenColor(col, amt) {
      ...
      if (col.startsWith("rgb")) {
        var parts = col.match(/rgba?\((\d{1,3})\s*,?\s*(\d{1,3})\s*,?\s*(\d{1,3})(?:\s*\/\s*(\d+\.?\d*%?))?\)/i);
        if (parts) {
          var _r = parseInt(parts[1]) + amt;
          var _g = parseInt(parts[2]) + amt;
          var _b = parseInt(parts[3]) + amt;
          var a = parts[4] ? parseFloat(parts[4]) : 1;
          _r = Math.min(255, Math.max(0, _r));
          _g = Math.min(255, Math.max(0, _g));
          _b = Math.min(255, Math.max(0, _b));

          // Conditionally format output depending on alpha presence
          if (parts[4]) {
            return "rgba(".concat(_r, ", ").concat(_g, ", ").concat(_b, ", ").concat(a, ")");
          } else {
...
class Interface {
      ...
      function modalTabGroups() {
        ...
        for (var key in window.CookieConsent.config.categories) {
          ...
          contentItems.push(el('dl.ccm__tabgroup' + '.' + key + (window.CookieConsent.config.categories[key].checked ? '.checked-5jhk' : ''), {
            ...
            'aria-label': "".concat(Language.getTranslation(window.CookieConsent.config.categories[key], window.CookieConsent.config.language.current, 'name'), " - tab")
  ...
  buildInitialModal() {
    return el('dialog#cconsent-init-modal', el('div.ccm__content', el('div.ccm__content__heading', el('h2#init__ccm__content__title', Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'modalMainTitle')), el('p', Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'modalMainText'), window.CookieConsent.config.modalMainTextMoreLink ? el('a', {
      href: window.CookieConsent.config.modalMainTextMoreLink,
      target: '_blank',
      rel: 'noopener noreferrer'
    }, Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'learnMore')) : null)), el('div.ccm__footer', el('button.consent-decline', Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'initModalBtnDisagree')), el('button.ccm__edit', Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'initModalBtnOptions')), el('button.consent-give', Language.getTranslation(window.CookieConsent.config, window.CookieConsent.config.language.current, 'initModalBtnAgree')))), {
      ...
      try {
        var _loop = function _loop() {
          ...
          button.addEventListener('click', function () {
            var _this$elements$bar4, _this$elements$bar5, _this$elements$bar6, _this$elements$modalI13, _this$elements$modalI14, _this$elements$modalI15;
            var buttonSettings = document.querySelector('.ccb__edit');
            var buttonConsentDecline = document.querySelector('.consent-decline');
      ...
      document.getElementById('ccm__footer__consent-modal-submit').addEventListener('click', function () {
        ...
        _this.buildCookie(function (cookie) {
          _this.setCookie(cookie, function () {
            var _this$elements$modalI10, _this$elements$modalI11, _this$elements$modalI12, _this$elements$bar, _this$elements$bar2, _this$elements$bar3;
            _this.elements['modal'].classList.remove('ccm--visible');
            _this.elements['modal'].setAttribute('aria-hidden', 'true');
            _this.elements['modal'].setAttribute('tabindex', '-1');
